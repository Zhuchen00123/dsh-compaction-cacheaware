...
function sanitizeSummarizationMessage(message) {
    if (message.role !== 'assistant' || !message.content.some((block) => block.type === 'reasoning'))
        return message;
...
export class CacheAwareCompactionEngine extends CompactionEngine {
    ...
    isRetriableSummarizeError(error) {
        ...
        if (typeof code === 'string' && /proxy_error|QUOTA|BAD_REQUEST|exhausted|unreachable|MAX_TOKENS|UNSUPPORTED_CONTENT/i.test(code))
            return true;
        ...
        if (error instanceof LlmError)
            return /proxy_error|exhausted|unreachable/i.test(error.code ?? '') || /proxy_error|exhausted|unreachable/i.test(msg);
        return false;
    }
