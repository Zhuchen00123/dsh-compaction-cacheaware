...
import { resolveCompactSpec, resolveConfig } from './config.js';
...
function conversationTarget(agent) {
    ...
    if (routed !== undefined)
        return routed;
    if (agent.options.provider === undefined || agent.options.provider.length === 0 || agent.options.model === undefined || agent.options.model.length === 0)
        return undefined;
...
function inspectCompactionEntryState(events) {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
        ...
        if (openTurnStateKnown && compactionEntryStateKnown && latestEndSeedSeq !== undefined)
            break;
    }
...
function assertCompactionInactive(unmatchedCompactionStart, latestEndSeedSeq, stage) {
    if (unmatchedCompactionStart === undefined || (latestEndSeedSeq !== undefined && latestEndSeedSeq > unmatchedCompactionStart.seq))
        return;
    throw new ManualCompactionError('busy', `${stage}: compaction already in progress; the session compaction lock is already active`);
}
function sanitizeSummarizationMessage(message) {
...
function portableBlockText(block) {
    switch (block.type) {
        case 'text':
            return block.text;
        case 'reasoning':
...
function buildPortableSummarizationInput(input) {
...
export class CacheAwareCompactionEngine extends CompactionEngine {
    ...
    sanitizeErrorMessage(raw) {
        ...
        if (/\b(api[_-]?key|token|secret|authorization|bearer|password)\b/i.test(trimmed))
            return 'authentication error';
        if (/https?:\/\/\S+/i.test(trimmed))
            return `${trimmed.replace(/https?:\/\/\S+/g, '[url]').slice(0, 220)}${raw.length > 220 ? '…' : ''}`;
    ...
    isNonCandidateRetryableError(error) {
        const msg = error instanceof Error ? error.message : String(error);
        if (/checkpoint rejected|summary is not smaller|SurfaceChanged|would not reduce tokens/i.test(msg))
            return false;
        return true;
    }
    isRetriableSummarizeError(error) {
        const msg = error instanceof Error ? error.message : String(error);
        if (/checkpoint rejected|summary is not smaller|SurfaceChanged|would not reduce tokens/i.test(msg))
            return false;
        const code = error?.code ?? error?.cause?.code;
        if (typeof code === 'string' && /proxy_error|QUOTA|BAD_REQUEST|exhausted|unreachable|MAX_TOKENS|UNSUPPORTED_CONTENT|INVALID_REQUEST/i.test(code))
            return true;
        if (/all upstreams exhausted|unreachable|proxy_error|invalid_prompt|invalid request|unsupported prompt|BAD_REQUEST|QUOTA|ECONN|ETIMEDOUT|ENOTFOUND|exhausted/i.test(msg))
            return true;
        // LlmError transport failures are retriable; keep logic conservative
        if (error instanceof LlmError)
            return /proxy_error|exhausted|unreachable|invalid_request/i.test(error.code ?? '') || /proxy_error|exhausted|unreachable|invalid_prompt/i.test(msg);
        return false;
    }
    async buildSummarizationCandidates(agent, signal) {
        const candidates = [];
        const seen = new Set();
    ...
    async summarizeWithCandidate(provider, model, input, agent, signal) {
        ...
        for await (const chunk of this.ctx.get('llm').stream(options))
        ...
        for (const candidate of budgetedCandidates) {
            try {
                ...
                if (typeof code === 'string')
                    sanitizedAttempts[sanitizedAttempts.length - 1] += ` (${code})`;
                ...
                if (this.isInvalidPromptError(error)) {
                    try {
                        return await this.summarizeWithCandidate(candidate.provider, candidate.model, buildPortableSummarizationInput(input), agent, signal);
                    }
                    catch (portableError) {
                        ...
                        if (!this.isNonCandidateRetryableError(portableError))
                            continue;
                        if (!this.isRetriableSummarizeError(portableError))
                            continue;
    ...
    async _selectRange(agent, measurement, force) {
        const target = routedTarget(agent.session);
    ...
    _rangeFromSeqs(agent, start, end) {
        ...
        if (!toolPairingBalancedBefore(agent.session, nodes[startIdx].seq))
            throw new Error(`compactRegion: start seq ${start} is not a balanced boundary`);
        if (!toolPairingBalancedAfter(agent.session, nodes[endIdx].seq))
    ...
    async _compactSurfaceRegion(agent, range, options, signal) {
    ...
    _commitCompactionBody(session, startEvent, summarized) {
    ...
    _assertWholeSurfaceUnchanged(session, range, preparedMeasurement) {
    ...
    _assertSelectedSpanStable(session, range, preparedMeasurement) {
        let current;
        try {
            ...
            if (!isDeepStrictEqual(currentSeqs, range.shadowedSeqs))
                throw new Error('span changed');
        }
        catch (error) {
            throw new SurfaceChangedError('compaction: the selected span is no longer a valid replacement target', { cause: error });
        }
    }
    async _specFor(agent, signal, preferredTarget) {
...
function throwManualFailure(failure) {
    ...
    if (failure.error instanceof Error && /all summarization candidates failed|summarization failed:/i.test(failure.error.message))
        throw failure.error;
